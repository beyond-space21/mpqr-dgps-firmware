...
{
    if (line == NULL || s_conn_handle == INVALID_CONN_HANDLE || s_notify_handle == 0 || !s_notify_enabled) {
        ESP_LOGW(TAG, "notify skipped (conn=%u handle=%u enabled=%d)",
                 (unsigned)s_conn_handle, (unsigned)s_notify_handle, s_notify_enabled ? 1 : 0);
...
static void handle_control_line(const char *line)
{
    ...
    if (cmd.type == ONBOARDING_CMD_WIFI_CONNECTED) {
        ESP_LOGI(TAG, "wifi_connected ack received from app");
...
static int gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    switch (event->type) {
        ...
        case BLE_GAP_EVENT_DISCONNECT:
            ...
            ESP_LOGI(TAG, "BLE disconnect; restarting advertising");
            (void)ble_onboarding_gatt_start_advertising();
            return 0;
        case BLE_GAP_EVENT_SUBSCRIBE:
            if (event->subscribe.attr_handle == s_notify_handle) {
                s_notify_enabled = event->subscribe.cur_notify != 0;
                ESP_LOGI(TAG, "Notify subscription changed: enabled=%d (prev=%d cur=%d)",
...
esp_err_t ble_onboarding_gatt_start_advertising(void)
{
    ...
    ESP_LOGI(TAG, "Advertising started (service FFF0)");
#endif
    return ESP_OK;
}
...
esp_err_t ble_onboarding_gatt_notify_softap(const wifi_softap_credentials_t *creds)
{
    ...
    char line[256];
    (void)onboarding_protocol_build_softap_ready(line, sizeof(line), creds);
    ESP_LOGI(TAG, "Sending softap_ready notify (ssid=%s)", creds->ssid);
