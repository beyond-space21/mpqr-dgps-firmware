...
void app_main(void)
{
    ...
    // Initializes with default settings -> Offsets read will be based on default units m/s^2, deg/s, and uT
    ...
    ESP_ERROR_CHECK(bno055_get_offsets(&bno055));

    ESP_ERROR_CHECK(bno055_get_readings(&bno055, LINEAR_ACCELERATION));

    bno055.config.offsets.accelerometer.x = bno055.linear_acceleration.x;
    bno055.config.offsets.accelerometer.y = bno055.linear_acceleration.y;
    bno055.config.offsets.accelerometer.z = bno055.linear_acceleration.z;

    ESP_ERROR_CHECK(bno055_set_offsets(&bno055));

    ESP_LOGI("BNO055", "Reading the sensor");
    while (1)
    {
        ESP_ERROR_CHECK(bno055_get_readings(&bno055, LINEAR_ACCELERATION));
        vTaskDelay(pdMS_TO_TICKS(100));
